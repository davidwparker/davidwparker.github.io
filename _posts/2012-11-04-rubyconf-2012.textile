...
h2(header). My Notes from the conference

h3(header). Matz's Keynote
...
h3(header). Implementation details of Ruby 2.0 VM - Koichi Sasada
...
h3(header). Ruby 2.0 (on Rails) - Akira Matsuda
...
h3(header). Building Data Driven products using Ruby - Ryan Weald
...
h3(header). Why JRuby Works - Charles Nutter, Tom Enebo
...
h3(header). Ruby vs the World - Matt Aimonetti
...
h3(header). Real Time Salami - Aaron Patterson
...
h3(header). Next Frontier of Deployment - Dr. Nic Williams
...
h3(header). Inside RubyMotion - Rich Kilmer
...
h3(header). Insufficiency of Good Design - Sarah Mei
...
h3(header). Game Development and Ruby - Andrew Nordman
...
h3(header). Diablo 3 bot with JRuby - Rodrigo Franco

Aka "Automating interface testing"

Sikuli - fantastic Java library used to test UI

h3(header). Simulating the world with Ruby - Bryan Liles
...
h3(header). Machine Learning - Chris Nelson

Decision Trees => ID3 (iterative dichotomiser)

gems: AI4R, DecisionTree

h3(header). An Intimate Chat with Matz
...
h3(header). _why, the luck stiff documentary
